...
Parameters.VS_RtGnSp = (-Affine_GenTorque+sqrt((Affine_GenTorque^2)+4*Parameters.VS_Slope25*0.99*Parameters.VS_RtPwr))/(2*Parameters.VS_Slope25); %Rated generator speed (HSS side) [rad/s] for constant torque in region 3
...
for genSpeed=0:0.0001:(PC_RefSpd)
         % TORQUE CONTROLLER
        ...
        if (genSpeed > Parameters.VS_RtGnSp)
            ...
            torque = Parameters.RatedGenTorque; %Constant torque
        ...
        if genSpeed*torque>0 && genSpeed*torque<=Parameters.VS_RtPwr
...
figure
plot(powerArrayOut, generatorSpeedArrayOut/Parameters.gbRatio)
xlabel('Power Command [W]')
ylabel('Rotor Speed [rad/s]')
%}

%%{
rotorSpeedArrayOut=generatorSpeedArrayOut/Parameters.gbRatio; % in rad/s
rotorSpeedInterpolant=griddedInterpolant(powerArrayOut,rotorSpeedArrayOut,'linear','linear');
